...
class _EcranTableauBordState extends State<EcranTableauBord> {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(border: Border.all(color: Colors.white, width: 2), borderRadius: BorderRadius.circular(6)),
          ...
          const Text("Quel Examen désirez-vous voir ?", style: TextStyle(color: Colors.white70, fontSize: 13)),
          const SizedBox(height: 15),
          Row(
            children: [
              Expanded(
                child: Container(
                  ...
                  child: TextField(
                    onChanged: (v) { rechercheMatiere = v; _refresh(); },
              ...
              PopupMenuButton<String>(
                icon: Container(
                  height: 45, width: 45, decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(10)),
  ...
  Widget _sectionTitle(String t, String route) {
    return Padding(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text(t, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800, color: vertExamino)),
          InkWell(
            onTap: () => Navigator.pushNamed(context, route),
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
              decoration: BoxDecoration(color: vertExamino.withOpacity(0.05), borderRadius: BorderRadius.circular(8), border: Border.all(color: vertExamino.withOpacity(0.3))),
              child: Text("Voir plus", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: vertExamino)),
            ),
          ),
        ],
      ),
  ...
  Widget _buildHorizontalList(List<ModeleExamen> list, bool recent) {
    if (list.isEmpty) return Container(height: 100, alignment: Alignment.center, child: Text("Aucun examen", style: TextStyle(color: Colors.grey.shade400)));
    return SizedBox(
      height: 185, 
      child: ListView.builder(
        ...
        itemBuilder: (c, i) => Container(
          ...
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(25), border: Border.all(color: orangeExamino, width: 3.5), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 8, offset: const Offset(0, 4))]),
          child: Column(
            mainAxisAlignment: MainAxisAlignment.center,
            children: [
              Container(
                padding: const EdgeInsets.only(bottom: 6), margin: const EdgeInsets.symmetric(horizontal: 15),
                decoration: const BoxDecoration(border: Border(bottom: BorderSide(color: Colors.black12, width: 1.5))),
                child: Text(list[i].titre.toUpperCase(), style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: vertExamino), textAlign: TextAlign.center),
              ),
              const SizedBox(height: 15),
              if (recent) ...[
                Text("Note: ${list[i].note ?? '?'}", style: const TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.black87)),
                const SizedBox(height: 4),
                const Text("En ligne", style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Colors.black54)),
              ] else ...[
  ...
  Widget _rowAvenirCentered(IconData icon, String text) {
    return Padding(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 16, color: Colors.black54), const SizedBox(width: 8), Text(text, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.black87))]),
    );
  }
      ...
      itemBuilder: (context, i) {
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(exam.titre, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)), Text(exam.date, style: const TextStyle(color: Colors.white70, fontSize: 13))]),
          ),
        );
      },
    );
